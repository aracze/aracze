...
const inputClass =
  'w-full rounded-xl border-[1.5px] border-line bg-white px-3.5 py-3 text-body text-ink outline-none transition focus:border-brand focus:ring-[3px] focus:ring-brand-tint'
...
function Message({ state }: { state: AccountFormState }) {
  ...
  return (
    <p
      ...
      className={`mb-4 rounded-xl px-4 py-3 text-small font-medium ${
  ...
  return (
    <form action={formAction} className="max-w-[420px]">
      ...
      <button
        type="submit"
        disabled={pending}
        className="mt-5 whitespace-nowrap rounded-full bg-brand px-7 py-2.5 font-heading text-label font-bold uppercase tracking-wider text-white transition-colors btn-halo disabled:cursor-not-allowed disabled:opacity-60"
      >
...
export function DeleteAccountForm({ publicName }: { publicName: string }) {
  ...
  if (!open) {
    return (
      <>
        <p className="mb-4 max-w-[560px] text-body leading-relaxed text-ink-2">
        ...
        <button
          ...
          className="rounded-full border-2 border-err/30 px-6 py-2 font-heading text-label font-bold uppercase tracking-wider text-err transition-colors hover:border-err"
        >
          Chci smazat účet
        </button>
      </>
    ...
    <form action={formAction} className="max-w-[520px]">
      ...
      <div className="rounded-2xl bg-err-bg p-5">
        ...
        <label
          ...
          className="mb-3 flex cursor-pointer items-start gap-2.5 text-small leading-snug text-ink"
        >
          <input
            id={removeNameId}
            name="removeName"
        ...
        <label
          htmlFor={confirmId}
          className="flex cursor-pointer items-start gap-2.5 text-small leading-snug text-ink"
        >
          <input
            id={confirmId}
            name="confirm"
      ...
      <div className="mt-5 flex items-center gap-3.5">
        <button
          type="submit"
          disabled={pending}
          className="whitespace-nowrap rounded-full bg-err px-7 py-2.5 font-heading text-label font-bold uppercase tracking-wider text-white transition-colors hover:brightness-90 disabled:cursor-not-allowed disabled:opacity-60"
        >
