    ...
    <div>
      <div className="relative">
        <input
          ...
          placeholder=" "
          className={`peer block w-full rounded-xl bg-surface px-4 pb-1.5 pt-5 text-body text-ink outline-none ring-2 transition-shadow focus:ring-brand/35 ${
  ...
  if (state.status === 'success') {
    return (
      <div className="mx-auto max-w-[340px] text-center">
        ...
        <p className="mt-4 text-label leading-relaxed text-ink-3">
  ...
  return (
    ...
    >
      ...
      {state.status === 'error' && (
        <p
          role="alert"
          className="mb-5 rounded-xl bg-err-bg px-4 py-3 text-center text-small font-medium text-err"
        >
          {state.message}
        </p>
      )}
      ...
      <button
        type="submit"
        disabled={pending}
        className="mx-auto mt-5 block w-fit rounded-full bg-brand px-9 py-3 font-heading text-small font-bold uppercase tracking-wider text-white transition-colors btn-halo disabled:opacity-60"
      >
